...
type Props = { params: { ville: string; metier: string } }
...
export function generateAlt({ params }: Props) {
  ...
  return `Site internet pour ${metier?.nom ?? 'professionnel'} à ${ville?.nom ?? 'Bretagne'} — DVS Web`
}
...
export default function OgImage({ params }: Props) {
  ...
  const nomMetier = metier?.nom ?? 'Professionnel'
    ...
    (
      ...
      >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          >
            Développeur web freelance — création de site sur-mesure pour les professionnels
          </div>
        </div>
